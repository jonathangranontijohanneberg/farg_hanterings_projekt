...
class TextField < Gosu::TextInput
  ...
  INACTIVE_COLOR  = Gosu::Color::RED
  ACTIVE_COLOR    = Gosu::Color::GREEN
  SELECTION_COLOR = Gosu::Color::BLUE
  ...
  def filter new_text
    ...
    new_text[2, allowed_length]
    new_text2[2, allowed_length]
    ...
    # if tab_counter == 0 || tab_counter == 3 || tab_counter == 6
    #   r_arr << new_text.to_i
    #   r = r_arr[(r_arr.lenth - 1) - 2] + r_arr[(r_arr.lenth - 1) - 1] + r_arr[(r_arr.lenth - 1)]
    #   p r
    # elsif tab_counter == 1 || tab_counter == 4 || tab_counter == 7
    #   g_arr << new_text.to_i
  ...
  def draw(z)
    ...
    Gosu.draw_rect x - PADDING, y - PADDING, WIDTH + 4 * PADDING, height + 4 * PADDING, color, z
...
class TextInputDemo < (Example rescue Gosu::Window)
  ...
  def button_down(id)
    ...
    elsif id == Gosu::KB_ESCAPE #|| id == Gosu::KB_Q
